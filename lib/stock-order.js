...
//   rows        — every stock_sectors row for the given list ('main' or
//                 'short-term'), sorted (sector group order, then
//                 position within that sector)
//   sectorOrder — the distinct sectors present, in display order
async function getOrderedStocks(listType = 'main') {
  const [stocksRes, sectorOrderRes] = await Promise.all([
    pool.query(`SELECT symbol, sector, sort_order FROM stock_sectors WHERE list_type = $1`, [listType]),
    pool.query(`SELECT sector, sort_order FROM sector_order WHERE list_type = $1`, [listType]),
  ]);
