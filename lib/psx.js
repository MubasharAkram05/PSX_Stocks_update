...
module.exports = { getStockPrice, getStockPriceWithTrend, getDailyRange };

// -------------------------------------------------------
// Returns the LOWEST and HIGHEST price recorded for the symbol so far
// today, using intraday tick data (not just the latest/current
// price). Falls back to the EOD close (used for both) if intraday
...
async function getDailyRange(symbol) {
  ...
  try {
    ...
    console.log(`Daily range lookup failed for ${sym}: ${err.message}`);
  }

  // Fallback: EOD close for both, if intraday ticks aren't available
  const plain = await getStockPrice(sym);
  return { symbol: plain.symbol, low: plain.price, high: plain.price, date: todayStr };
}
