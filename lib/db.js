...
async function ensureSchema() {
  ...
  // --- Manual reordering / removal for the Recently Added page ---
  // Recently Added is otherwise a computed view over save history (last
  // N distinct symbols by save recency); this table lets that order be
  // overridden and lets a symbol be dismissed from the list without
  // touching any saved price history.
  await pool.query(`
    CREATE TABLE IF NOT EXISTS recent_meta (
      symbol TEXT PRIMARY KEY,
      sort_order INTEGER,
